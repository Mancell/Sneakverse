...
export default async function AdminBlogPage({
  ...
  const { posts, total, page, totalPages } = await getAdminBlogPosts({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-heading-1 text-dark-900">Blog Posts</h1>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
            <Link
              key={pageNum}
              href={`/admin/blog?page=${pageNum}${params.search ? `&search=${params.search}` : ""}${params.isPublished ? `&isPublished=${params.isPublished}` : ""}`}
